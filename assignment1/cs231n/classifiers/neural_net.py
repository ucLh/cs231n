...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # shape (N, C).                                                             #
        #############################################################################
        linear_scores = X.dot(W1) + b1
        layer1_scores = np.maximum(linear_scores, 0)
        ...
        # backpropagation
        dout = np.exp(scores)  # (N, C)
        dout /= np.sum(dout, axis=1, keepdims=True)
        dout[np.arange(N), y] -= 1
        dW2 = np.dot(layer1_scores.T, dout)  # (H, C)
        d_layer1 = np.dot(dout, W2.T) * (linear_scores > 0)  # (N, H)
        dW1 = np.dot(X.T, d_layer1)  # (D, H)

        grads['W2'] = dW2 / N
        grads['b2'] = np.sum(dout, axis=0) / N
        grads['W1'] = dW1 / N
        grads['b1'] = np.sum(d_layer1, axis=0) / N
    ...
    def predict(self, X):
        ...
        scores = layer1_scores.dot(W2) + b2  # (N, C)
