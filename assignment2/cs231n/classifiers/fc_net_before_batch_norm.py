...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        layer1_scores, layer1_cache = affine_relu_forward(X, W1, b1)
        scores, layer2_cache = affine_forward(layer1_scores, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dX2, dW2, db2 = affine_backward(dout, layer2_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        pre_last_layer = layers_scores[self.num_layers - 2]
        W_last = self.params["W" + str(self.num_layers)]
        b_last = self.params["b" + str(self.num_layers)]

        scores, last_layer_cache = affine_forward(
            pre_last_layer, W_last, b_last)

        layers_scores.append(scores)
        layers_cache.append(last_layer_cache)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        last_ind = self.num_layers
        grads['X' + str(last_ind)], grads['W' + str(last_ind)], grads['b' + str(last_ind)] = \
            affine_backward(dout, last_layer_cache)
